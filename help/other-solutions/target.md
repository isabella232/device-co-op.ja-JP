...
Device Co-opデータは、Device Co-opアクティビティ、Recommendationアクティビティまたはアクティビティで、をレポートソース(A4Tと呼ばれる、 [!DNL Adobe Analytics] および [!DNL Target][!DNL Analytics] 統合)として使用することはできません。

>[!NOTE]
>
>必要なバージョンのがあることを確認し `mbox.js`ます。 任意のバージョンのを使用でき `at.js`ます。 詳しくは、 [メンバーシップ要件を参照してください](../about/requirements.md#concept-31d3d165d22546afbedf023d32ad3a43)。

## デバイスに関係なく、関連するコンテンツを配信 {#section-bba8d41e96914c82a6d267a54f776354}
...
Device Co-opを使用すると、ユーザの様々なデバイスを同じユーザが使用していると識別できます。 ユーザーが [!DNL Target][!DNL Target] アクティビティを含むページ(アクティビティまたはターゲットコンテンツ)を表示すると、が別のデバイスで見たのと同じエクスペリエンスを確実に閲覧できるようになります。

## 訪問者ではなく人別にターゲットアクティビティを分析する {#section-c25cf4f8483942d7836d60756235e62c}

マーケターは、「訪問者」ではなく「人」で [!DNL Target] アクティビティを分析したいと考えています。

各ユーザーは、デバイスやブラウザー間で同じ会社やブランドを操作する可能性が高くなりますが、Device Co-opを使用しない場合、各デバイスやブラウザーは、 [!DNL Target] レポートでは個別の「訪問者」と見なされます。
...
この概念について詳しくは、 [Analyticsを参照してください。人物指標](../other-solutions/people.md#concept-8c57cd3904974e078d7fbf84ac9c2d63)。

## アクティビティごとのDevice Co-opデータの使用 {#section-fb46fae482654023abb1a1e26564db9a}

マーケターは、アクティビティごとにDevice Co-opデータを使用するように選択できます。 次のようなDevice Co-opデータには、一部の [!DNL Target] アクティビティが適していない場合があります。
...
でオーディエンスを作成すると、Device Co-opのデータ対応アクティビティにオーディエンスが適切でない場合に、その旨が警告されます。 [!DNL Target]適切なオーディエンスには、すべての訪問者、新しい訪問者、再訪問者が含まれます。
